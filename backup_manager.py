...
def increment_and_check_backup():
    ...
    if current_count >= NEW_CUSTOMER_THRESHOLD:
        ...
        try:
            ...
            st.success("Backup automático do banco de dados enviado para o Google Drive com sucesso!")
            save_counter(0) # Reseta o contador após o backup bem-sucedido
            # Pequena pausa para a mensagem de sucesso aparecer antes do Streamlit rerender
            time.sleep(2)
        except google_drive_service.GoogleDriveServiceError as e:
            st.error(f"Erro no backup automático para o Google Drive: {e}")
        except Exception as e:
            st.error(f"Ocorreu um erro inesperado durante o backup automático: {e}")
        finally:
