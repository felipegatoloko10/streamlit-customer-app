...
DEFAULT_CALC_INPUTS = {
    ...
    # Estados de expansão
    'expand_impressao': False, 'expand_fatores': False
}
DEFAULT_PRESETS = {}
...
def calculate_costs(inputs):
    ...
    return { "Custo de Mão de Obra Total": total_labor_cost, "Custo de Material": cost_material,
...
st.write("Preencha os campos abaixo de forma progressiva para gerar o preço de venda.")

with st.expander("💾 Gerenciar Predefinições", expanded=False):
    ...
    with col_load:
        if st.button("Carregar", disabled=not selected_preset):
    ...
    with col_delete:
        ...
        if st.button("🗑️", disabled=not selected_preset, help="Excluir predefinição selecionada"):
    ...
    if save_modal.is_open():
        with save_modal.container():
            ...
            if st.button("Salvar Predefinição", type="primary"):
                if new_preset_name:
                    current_inputs_for_preset = {key: st.session_state[key] for key in DEFAULT_CALC_INPUTS if 'expand_' not in key}
