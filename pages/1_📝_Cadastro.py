...
DEFAULT_VALUES = {
    ...
    # Estados de expansão
    "expand_dados": False, "expand_contatos": False, "expand_endereco": False, "expand_obs": False
}
...
with st.expander("Passo 1: Busca de Endereço por CEP", expanded=True):
...
with st.expander("Passo 2: Dados Principais", expanded=st.session_state.expand_dados):
...
with st.expander("Passo 3: Contatos", expanded=st.session_state.expand_contatos):
    ...
    st.text_input("Cargo do Contato 1", key="widget_cargo_input", on_change=lambda: expand_section("endereco"))
    st.markdown("---")
